//-----------------------------------------------------------------------
// <copyright file="ACRCloudReference.cs" company="ACRCloud">
...
// </copyright>
//-----------------------------------------------------------------------
// Snippets taken from recognizer.cs from the ACRCloud C# Samples.
/*
   @author qinxue.pan E-mail: xue@acrcloud.com
   @version 1.0.1
   @create 2015.10.01
...
{
    internal class ACRCloudExtrTool
    {
    ...
    internal class ACRCloudRecognizer
    {
        ...
        public StreamContent Recognize(byte[] ext_fp, DateTime? dateTime = null, string timestampIn = null)
        {
            IDictionary<string, Object> query_data = new Dictionary<string, Object>();
            query_data.Add("ext_fp", ext_fp);
            return this.DoRecognize(query_data, dateTime, timestampIn);
        }

        private StreamContent PostHttp(string url, DateTime dateTime, IDictionary<string, Object> postParams)
        {
            string result = "";

            //string BOUNDARYSTR = "acrcloud***copyright***2015***" + DateTime.Now.Ticks.ToString("x");
            string BOUNDARYSTR = "acrcloud___copyright___2015___" + dateTime.Ticks.ToString("x", CultureInfo.InvariantCulture);
            ...
            foreach (var item in postParams)
            {
                ...
                {
                    string tmpStr = string.Format(CultureInfo.InvariantCulture, stringKeyHeader, item.Key, item.Value);
                    byte[] tmpBytes = Encoding.UTF8.GetBytes(tmpStr);
                    memStream.Write(tmpBytes, 0, tmpBytes.Length);
                }
                else if (item.Value is byte[])
                {
                    var header = string.Format(CultureInfo.InvariantCulture, filePartHeader, item.Key, item.Key);
            ...
            try
            {
                ...
                bool dummy = result.Contains("result", StringComparison.InvariantCulture);
                memStream.Seek(0, SeekOrigin.Begin);
                StreamContent streamContent = new StreamContent(memStream);
                streamContent.Headers.TryAddWithoutValidation("Content-Type", "--multipart /form-data; boundary=" + BOUNDARYSTR);
                streamContent.Headers.ContentLength = memStream.Length;
            ...
            catch (WebException e)
            {
                ...
                throw new Exception("ACRCloudStatusCode.HTTP_ERROR", e);
            }
            catch (Exception e)
            {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Security", "CA5350:Do Not Use Weak Cryptographic Algorithms", Justification = "Code by others.")]
        ...
        private StreamContent DoRecognize(IDictionary<string, Object> query_data, DateTime? dateTimeIn, string timestampIn)
        {
            byte[] ext_fp = null;
            byte[] hum_fp = null;
            string method = "POST";
            ...
            DateTime dateTime = dateTimeIn.HasValue ? dateTimeIn.Value : DateTime.Now;
            string timestamp = string.IsNullOrEmpty(timestampIn)
                ? ((int)dateTime.Subtract(new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc)).TotalSeconds).ToString(CultureInfo.InvariantCulture)
                : timestampIn;
            ...
            if (query_data.ContainsKey("ext_fp")) {
                ...
                if (ext_fp != null)
                {
                    dict.Add("sample_bytes", ext_fp.Length.ToString(CultureInfo.InvariantCulture));
            ...
            if (query_data.ContainsKey("hum_fp"))
            {
                ...
                if (hum_fp != null)
                {
                    dict.Add("sample_hum_bytes", hum_fp.Length.ToString(CultureInfo.InvariantCulture));
            ...
            if (ext_fp == null && hum_fp == null)
            {
                ...
                throw new Exception("ACRCloudStatusCode.NO_RESULT");
            }
            ...
            StreamContent res = PostHttp(reqURL, dateTime, dict);
